...
export const orderDetailsReducer = (state = { loading: true }, action) => {
    switch (action.type)
    {
        case ORDER_DETAILS_REQUEST:
            return {...state, loading: true}; //when we initalize empt obj we need to expand the state.
...
switch (action.type)
{
    ...
    case ORDER_MY_LIST_FAIL:
        return {loading:false, error: action.payload};
    case ORDER_MY_LIST_RESET: // we dont do reset in action, we do it component
        return { orders: []}; // watever u have mentioned in default mention here to rest it to default value.
    default:
        return state;
}
};
